...
void MainWindow::wheelEvent(QWheelEvent *event){
  ...
  if (ui->mdiArea->underMouse()){
    if(dl_alloc != nullptr){dl_alloc->wheelEvent(event);}
    if(ul_alloc != nullptr){ul_alloc->wheelEvent(event);}
    if(diff_alloc != nullptr){diff_alloc->wheelEvent(event);}
    if(dl_spec != nullptr){dl_spec->wheelEvent(event);}
  }
}
