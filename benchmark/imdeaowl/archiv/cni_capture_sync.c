...
int main(int argc, char **argv) {
  ...
  datatransfer_thread_t* h_transfer = NULL;
  ...
  int len = strlen(prog_args.output_file_name);
  ...
  network_info_t* netinfo_before = NULL;
  network_info_t* netinfo_after = NULL;
  ...
  while (!go_exit && (sf_cnt < prog_args.nof_subframes || prog_args.nof_subframes == -1)) {
    ...
    //	  go_exit = true;
    //    }
    if (h_transfer == NULL && fstart && sf_cnt > prog_args.probing_delay) {
      fprintf(stderr, "Start probing transfer of %d to/from %s\n", prog_args.payload_size, prog_args.url);
      h_transfer = uplink_probe(prog_args.payload_size, prog_args.url, NULL, NULL);
    }
    if (h_transfer != NULL) {
