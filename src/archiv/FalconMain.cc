...
srsue::metrics_stdout metrics_screen;
...
int main(int argc, char** argv) {
  cout << "Welcome to Falcon" << endl;
  Falcon& falcon = Falcon::instance();

  srslte::metrics_hub<srsue::ue_metrics_t> metricshub;
  ...
  CCSniffer* ccSniffer = CCSniffer::get_instance();

  if (!ccSniffer) {
    cout << "Error creating Sniffer instance." << endl << endl;
    exit(1);
  }

  cout << "Starting Sniffer" << endl;
  if(!ccSniffer->init(&args)) {
    exit(EXIT_FAILURE);
  }

  metricshub.init(ccSniffer, args.expert.metrics_period_secs);
  metricshub.add_listener(&metrics_screen);
  metrics_screen.set_ue_handle(ccSniffer);

  srsue::metrics_csv metrics_file(args.expert.metrics_csv_filename);
  if (args.expert.metrics_csv_enable) {
    metricshub.add_listener(&metrics_file);
    metrics_file.set_ue_handle(ccSniffer);
  }
  ...
  printf("Attaching UE...\n");
  while (!ccSniffer->switch_on() && running) {
    sleep(1);
  }
  ...
  if (running) {
    if (args.expert.pregenerate_signals) {
      ...
      ccSniffer->pregenerate_signals(true);
      printf("Done pregenerating signals.\n");
    }
    if (args.gui.enable) {
      ccSniffer->start_plot();
    }
  ...
  while (running) {
    ...
    if(mbms_service_start) {
      if(ccSniffer->mbms_service_start(serv, port)){
        mbms_service_start = false;
      }
    }
    if(show_mbms) {
      show_mbms = false;
      ccSniffer->print_mbms();
    }
    if (args.expert.print_buffer_state) {
      cnt++;
      if (cnt==10) {
        cnt=0;
        ccSniffer->print_pool();
      }
    }
    sleep(1);
  }

  ccSniffer->switch_off();
